...
# set parameter
params = heaan.HEParameter.from_preset("FVc")
...
#===================================================================
program = "HESFLV2 ResNet18 on HAM10000"
...
class ResNet18_server_side(nn.Module):
    def __init__(self, block, num_layers, classes):
        ...
        self.fc = nn.Linear(2048 * block.expansion, classes)
...
def EncFedAvg(w):
    st = time.time()
    w_avg = copy.deepcopy(w[0])
    mw = heaan.Block(context,encrypted = False, data = [1/len(w)]*(num_slot))
    w_dict = {}
    for k in w_avg.keys():
        w_avg_li = w_avg[k].flatten().tolist()
        # print('======= first thing ========')
        # tensor_shape = w_avg[k].shape
        #print(len(w_avg_li), tensor_shape)
        # print(w_avg_li)
        m0 = heaan.Block(context,encrypted = False, data = w_avg_li)
        tmp1 = m0.encrypt()
        for i in range(1, len(w)):
            w_li = w[i][k].flatten().tolist()
            # print('======= second thing ========')
            # print(len(w_li), w_li)
            m1 = heaan.Block(context,encrypted = False, data = w_li)
            tmp2 = m1.encrypt()
            tmp1 = tmp2 + tmp1
        # m =  tmp1.decrypt(inplace=False)
        # print("================= third thing ==================")
        # for i in range(len(w_avg_li)):
        #     print(m[i].real)
        tmp1 = tmp1 * mw
        # m =  tmp1.decrypt(inplace=False)
        # print("================= fourth thing ==================")
    ...
    for k in w_dict.keys():
        m = w_dict[k].decrypt(inplace=False)
        li = []
        w_avg_li = w_avg[k].flatten().tolist()
        for i in range(len(w_avg_li)):
            li.append(m[i].real)
        # m = m.tolist()
        new_tensor = np.array(li).reshape(w_avg[k].shape)
        w_dict[k] = torch.tensor(new_tensor)
    ...
    #     # 두 텐서가 같은지 확인
    #     are_equal = torch.equal(w_dict[k], w_avg[k])

    #     # 결과 출력
    #     print(f"Are the tensors equal? {are_equal}")
        
    #w_avg = w_dict.state_dict()
    #w_avg = OrderedDict(w_dict)
    return w_dict, fed_time
...
for iter in range(epochs):
    ...
    for idx in idxs_users:
        ...
        te_en = time.time()
        avg_te_time += (te_en-te_st)
    tr_time_li.append(round(avg_tr_time/num_users,3))
    te_time_li.append(round(avg_te_time/num_users,3))
    ...
    print("-----------------------------------------------------------")
    w_glob_client, Fed_time = EncFedAvg(w_locals_client)
    Fed_li.append(Fed_time)   
    
    # Update client-side global model 
    net_glob_client.load_state_dict(w_glob_client)    
    epoch_en = time.time()
    epoch_time = epoch_en - epoch_st
    epoch_li.append(epoch_time)
...
print(len(round_process), len(acc_train_collect), len(acc_test_collect), len(tr_time_li), len(te_time_li), len(Fed_li), len(epoch_li))
df = DataFrame({'round': round_process,'acc_train':acc_train_collect, 'acc_test':acc_test_collect, 'training time':tr_time_li, 'test time':te_time_li, 'Fedavg time':Fed_li, 'epoch time':epoch_li})     
